...
TARGETS = gb-ezflashjr gb-everdrivex
# TARGETS = gb-ezflashjr # i only have this one
# clean gb-ezflashjr is not deleting the folder correctly
...
LCCFLAGS += -Wl-j -Wm-yoA -autobank -Wb-ext=.rel -Wb-v
...
ifdef FAKE
CFLAGS	 += -DSD_READ_EMULATION -DTXT_READ_EMULATION #simulate stuff
PFF_STUB := 1
endif

# CFLAGS   += -debug
# CFLAGS += --more-pedantic
# CFLAGS   += -Wf'--max-allocs-per-node 0'
...
# ???: obj/gb-ezflashjr/txt_player.asm:1887: Error: <a> Invalid Addressing Mode.

# You can set the name of the ROM file here
PROJECTNAME = txt_reader
...
ifdef PFF_STUB
  $(info *** Petit FS disabled: removing 'pff.c' from CSOURCES ***)
  $(info *** if you disabled by mistake, set PFF_STUB = 0 @ this makefile ***)
  CSOURCES := $(filter-out pff.c,$(CSOURCES))
else
  $(info *** Petit FS enabled: removing 'pff_stub.c' from CSOURCES ***)
  CSOURCES := $(filter-out pff_stub.c,$(CSOURCES))
endif
...
.PRECIOUS: $(CSOURCES) $(RESDIR)/%.c $(RESDIR)
