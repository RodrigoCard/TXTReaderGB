...
const uint8_t * const EXTENSIONS[MAX_EXTENSIONS] = {".TXT", ".MD", ".CFG", ".INI"};
...
bool browser_show_all_extensions = false;
const menu_item_t * browser_last_selection;
char last_filename[13] = "\0"; // TODO: Recent files

static void drawStatusBar(void) {
    strcpy(text_buffer, ICON_A " Open ");
    // strcpy(text_buffer, ICON_A " Open " ICON_START " Info "); // TODO: put credits on start
    if(browser_show_all_extensions) {
        strcat(text_buffer, ICON_SELECT " all files");
    } else {
        strcat(text_buffer, ICON_SELECT " txt files");
    }
    
    vwf_activate_font(FONT_VARW);
    menu_text_out(0, 17, DEVICE_SCREEN_WIDTH, BLACK_ON_LT_GR, ITEM_TEXT_CENTERED, text_buffer);
    vwf_activate_font(FONT_FIXED6);
}
...
uint8_t onTranslateKeyFileBrowser(const struct menu_t * menu, const struct menu_item_t * self, uint8_t value) {
    ...
    if (value & J_SELECT) {
        browser_show_all_extensions = !browser_show_all_extensions;
        browser_menu_result = ACTION_READ_DIRECTORY;
        drawStatusBar();
    }
    if (value & J_LEFT) {
...
uint8_t onShowFileBrowser(const menu_t * self, uint8_t * param) {
    ...
    if (len == 0) {
        strcpy(text_buffer, APP_NAME_STR " " APP_VERSION_STR);
    } else {
        strcpy(text_buffer, "[/");
        if (len > 25) {
            strcat(text_buffer, "...");
    ...
    vwf_activate_font(FONT_VARW);
    menu_text_out(0, 0, DEVICE_SCREEN_WIDTH, BLACK_ON_LT_GR, ITEM_TEXT_CENTERED, text_buffer);
    #if defined(MASTERSYSTEM)
...
bool check_ext_arr(const uint8_t * str) {
    if (browser_show_all_extensions) return true;
    for (uint8_t i = 0; i < MAX_EXTENSIONS; i++) {
        bool r = check_ext(EXTENSIONS[i], str);
        if(r) return true;
    }
    return false;
}
...
uint8_t read_directory(uint8_t * path) {
    ...
    while (files_loaded < 89) {
        fn = files[files_loaded] = files_list + files_loaded;
        if (((files_loaded+1) & 7) == 0) {
            strcpy(fn->fname, "somedir");
            fn->fattrib = AM_DIR;
        } else {
            strcpy(fn->fname, "test.txt");
            fn->fattrib = 0;
        }
        files_loaded++;
    }
    ...
    while (true) {
        ...
        if (!fn->fname[0]) break;
        if ((fn->fattrib & AM_DIR) || check_ext_arr(fn->fname)) {
    ...
    do {
        ...
        switch (menu_result) {
            case ACTION_EXECUTE_FILE:
                // TODO: Recent files
                strcpy(last_filename, browser_last_selection->caption);
                sprintf(text_buffer, "Reading: %s", last_filename);
                DrawMessageBox(text_buffer);
#if defined(SD_READ_EMULATION) && !defined(TXT_READ_EMULATION)
                MessageBox(TXT_ERRORS[TXT_READ_ERROR]);
#else // TXT_READ_EMULATION needs the real handling code as it is emulating pff behavior
                sprintf(text_buffer, "%s/%s", current_path, last_filename);
                TXT_RESULT tres = txt_play_file(text_buffer, last_filename);
                if (tres != TXT_OK) {
                    sprintf(text_buffer, "TXT: %s", TXT_ERRORS[tres]);
