...
inline const menu_item_t * menu_first(const menu_t * menu) {
    return menu->items;
}
...
uint8_t menu_execute(const menu_t * menu, uint8_t * param, const menu_item_t * select) {
    ...
    do {
        ...
        } else if (KEY_PRESSED(J_B)) { // shortcut to first item, often '..', so easier to go up a dir
            if ((new_selection = menu_first(menu)) != selection) {
                selection = menu_move_selection(menu, selection, new_selection);
                if (menu->onHelpContext) menu->onHelpContext(menu, selection);
            }
